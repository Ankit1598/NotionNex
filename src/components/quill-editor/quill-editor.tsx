	...
	return (
		<>
			<div className='relative'>
				{details.inTrash && (
					<article className='py-2 z-40 bg-red-500 flex md:flex-row flex-col justify-center items-center gap-4 flex-wrap'>
			...
			<div className='group/banner relative w-full h-[200px]'>
				{details.bannerUrl && (
					<>
						<Image
							src={
								supabase.storage
									.from("file-banners")
									.getPublicUrl(details.bannerUrl).data
									.publicUrl
							}
							fill
						...
						<Button
							...
							variant='outline'
							className='absolute top-2 right-2 gap-2 hover:bg-background flex item-center justify-center text-sm w-36 p-2 rounded-md'
						>
							<XCircleIcon size={16} />
				...
				<BannerUpload
					...
					dirType={dirType}
					className={twMerge(
						"absolute left-1/2 -translate-x-1/2 text-sm p-2 transition-all ease-in-out duration-200 rounded-md",
						details.bannerUrl
							? "bottom-0 hidden group-hover/banner:block"
							: "top-1/2 -translate-y-1/2"
					)}
				>
					<Button
						variant='outline'
						className='hover:bg-background flex item-center justify-center text-sm w-max rounded-md'
					>
